...
public class NFastenersInsideSleeperWTol implements ConstraintPattern {

    private final int N;
    private final int num_tol;
    private final int pos_tol;

    public NFastenersInsideSleeperWTol(int N, int num_tol, int pos_tol) {
    ...
    public void apply(CPSolver cp, Instance instance) {
        ...
        for (int i : filtered_values) {
            ...
            try {
                ...
                search.onSolution(() -> {
                    System.out.print("Sleeper " + sleeper_box_id + " -> Fasteners: ");
                    for (int j = 1; j <= tolerated_N; j++) {
                        CPIntVar fastener_box_id = element(box_ids, pattern[j]);
                ...
                System.out.println("Not enough fasteners for sleeper " + i);
            } finally {
